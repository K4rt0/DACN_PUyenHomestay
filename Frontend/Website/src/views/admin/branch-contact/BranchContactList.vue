<template>
    ...
    <div class="col-lg-8 col-8 col-sm-8 p-0 pe-2">
      <div class="card card-flush h-100">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h4 class="mb-0">Điều chỉnh liên hệ chi nhánh</h4>
        ...
        <div class="card-body pt-0">
          <div class="form-group mb-5">
            ...
            <input type="text" v-model="icon_preview" class="form-control form-control-solid mb-3" placeholder="fa-sharp-duotone fa-solid fa-earth-america" />
            <p class="mb-1 text-muted">Hướng dẫn: tìm kiếm và ấn vào Icon cần thêm vào và làm như sau:</p>
            <p class="mb-1 text-muted">- Bạn sẽ thấy đoạn code sau hiển thị lên <b>&lt;i class="fa-sharp-duotone fa-solid fa-earth-america"&gt;&lt;/i&gt;</b></p>
            <p class="mb-1 text-muted">- Hãy lấy <b>fa-sharp-duotone fa-solid fa-earth-americas</b> dán vào đây.</p>
          ...
          <div class="form-group mb-5">
            ...
            <input type="text" v-model="contact_name" class="form-control form-control-solid mb-3" placeholder="Số điện thoại, Facebook, Zalo,..." />
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="card">
    ...
    <div v-else-if="!loaded && items.length == 0" class="d-flex flex-column align-items-center" style="padding: 100px 0">
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="text-start">
                <i :class="[item.contact_icon, 'pe-3']" style="font-size: 30px"></i>
                <span class="fw-bold">{{ item.contact_icon }}</span>
              </td>
              <td class="text-end">
...
breadcrumb([{ path: "admin_branch_contact_list", name: "Branch Contact List" }]);

document.title = "Branch Contact List";

const icon_preview = ref("fa-sharp-duotone fa-solid fa-earth-america");
const contact_name = ref("");
const editing = ref(0);
const edit_loading = ref(false);

const edit_save = async () => {
  if (!icon_preview.value || !contact_name.value) toast.error("Vui lòng nhập đầy đủ thông tin !");
  else {
    ...
    if (editing.value === 0) {
      try {
        const response = await axios.post(
          "/branch-contact",
          {
            contact_icon: icon_preview.value,
...
const edit_del = async (item) => {
  if (confirm("Bạn có chắc chắn muốn xóa liên hệ này không ?")) {
    try {
      const response = await axios.delete(`/branch-contact/${item.id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwt_admin_token")}`,
        },
      });
...
const edit_cancel = () => {
  editing.value = 0;
  icon_preview.value = "fa-sharp-duotone fa-solid fa-earth-america";
  contact_name.value = "";
};

const edit_item = (item) => {
  editing.value = item.id;
  icon_preview.value = item.contact_icon;
  contact_name.value = item.name;
  toast.info("Bạn đã chọn liên hệ này, hãy chỉnh sửa nó ở trên !");
};
...
const filterOption = ref(["All", "Latest Updated", "Oldest Updated"]);
...
const fetchData = async () => {
  loading.value = true;
  try {
    const response = await axios.get("/branch-contact/get-pagination", {
      params: {
