...
namespace Backend.Controllers
{
    ...
    public class BranchController : ControllerBase
    {
        ...
        [HttpGet("get-optimal")]
        public async Task<ReturnApi> GetBranchForFooterAsync()
        {
            if (!ModelState.IsValid)
                return new ReturnApi(false);
            
            var branches = await _branchService.GetBranchForFooterAsync();
        ...
        public async Task<ReturnApi> CreateBranchAsync([FromBody] Branch branch)
        {
            ...
            if(await _branchService.IsBranchExistAsync(branch.name!))
                return new ReturnApi(false, "Chi nhánh này đã tồn tại !");
